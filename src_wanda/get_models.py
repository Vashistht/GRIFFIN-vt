...
def main(dataset='xsum', shots=1, model_arch='llama2', model_size=0, cache_dir=None, density=0.5,
         selection_method='topk', sample_num=1, max_length=-1, k=0, max_tokens=64, seed=42, temp=0.3,
         greedy=False, device='cuda:3', forward= True, select_every=None, 
         wanda=False): # @vashisthtiwari 
    ...
    if wanda is False:
        ...
        print('loading wanda model')
        # wanda_path = '/scratch/vash-experiment/wanda/sparsity-0.5'
        # wanda_path = '/home/vashistt/Desktop/pruned_models/llama_7b/unstructured/wanda/2-4'
        wanda_path = 'neuralmagic/Llama-2-7b-pruned50-retrained'
        config = AutoConfig.from_pretrained(wanda_path)
        tokenizer = AutoTokenizer.from_pretrained(wanda_path, use_fast=True)
        model = AutoModelForCausalLM.from_pretrained(wanda_path, torch_dtype=torch.float16)

        # home/vashistt/Desktop/pruned_models/llama_7b/unstructured/wanda/sparsity-0.5
    ...
    if adapter_path is True:
        ...
        adapter_path = "/home/vashistt/Desktop/pruned_models/llama_7b/unstructured/wanda/ft_0.5_old"
        model= PeftModel.from_pretrained(model, adapter_path, adapter_name="ft_c4")
        print('adapter')
    else:
        print('no adapter')
    ...
    with torch.torch.inference_mode(): # used to be no_grad
                ...
                if args.forward == True:
                    ...
                    if select_every is not None:
                        while len(input_ids[0]) < original_input_len+ max_tokens:
                            input_ids = autoregressive_sampling(input_ids, model, select_every, temperature, args.k, 1)
                            # print('input_ids:', input_ids.shape)
                    else:
                        print('normal expert selection')
        ...
        print("-----------------------------------------------------------")
        print(f"FINAL RESULTS, for {len(requests)} requests, with select_only {select_every} and max_tokens {max_tokens}:")
        ...
        print("-----------------------------------------------------------")
        end_time = time.time()
        print(f"Time taken: {end_time - start_time} for wanda {wanda} and requests {len(requests)}")
    '''
    return model
